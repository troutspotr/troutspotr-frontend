...
export const buildTableOfContentsEndpoint = (): string => `/data/v4/TableOfContents.topojson`
...
export const updateRegionCachedStatus = (
  ...
  const key = `/data/v4/${region.properties.path}.topojson`
...
export const updateCacheStatusForItems = (
  ...
  try {
    ...
    tableOfContents.states.features.forEach(state => {
      const key = `/data/v4/${state.properties.short_name}/${state.properties.short_name}.data.json`
      state.properties = {
        ...state.properties,
