...
import {countiesDictionarySelector, searchTextSelector, selectedRegionSelector} from 'ui/core/Core.selectors'
...
import {displayedCentroidDictionarySelector,
  ...
  regionIndexSelector, regulationsSelector,
  waterOpenersDictionarySelector} from 'ui/@state/State.selectors'
...
export const visibleTroutStreams = createSelector(
  ...
  (
    ...
    const filteredStreams = streamArray.filter((streamItem) => has(displayedDictionary, streamItem.stream.properties.gid))
    return filteredStreams
  })

export const visibleTroutStreamIdsSelector = createSelector(
  [visibleTroutStreams],
  (visibleStreams) => visibleStreams.map((s) => s.stream.properties.gid))
...
export const getSpecialRegulationsSelector = createSelector(
  ...
  (selectedStream, regulations) => {
    ...
    const specialRegulationsDictionary = selectedStream.restrictions.map((r) => {
      const {stream_gid, restriction_id, start, stop, end_time, start_time, color} = r.properties
...
export const getCountyListSelector = createSelector(
  ...
  (
    ...
    const countyObjects = countyIdsInSelectedRegion.map((id) => {
      ...
      const visibleStreamIdsInCounty = streamIdsInRegion.filter((streamId) => has(visibleTroutStreamDictionary, streamId))
      const visibleStreamsInCounty = visibleStreamIdsInCounty.map((streamId) => visibleTroutStreamDictionary[streamId])
      ...
      try {
        const countyList = {
          ...
          'streams': sortBy(visibleStreamsInCounty, [
            byOpenStatus,
            byName,
          ]),
        }
    ...
    const filteredCountyObjects = countyObjects.filter((x) => x.streams.length > 0)
