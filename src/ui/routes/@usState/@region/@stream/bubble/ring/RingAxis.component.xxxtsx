...
class RingAxisComponent extends React.Component<{}> {
  public renderTicks() {
    ...
    return times(ticks + 1).map(index => {
      ...
      const textAnchor = rotationDegrees > 90 ? 'middle' : 'middle'
      ...
      return (
        <g key={index + rotatePhase} transform={transform}>
          <line
            transform={`rotate(${rotationDegrees}) translate(-10, 0)`}
            className={ringClasses.radialGuide}
            x1={-2}
            x2={2}
          />
          <g>
