...
export const streamSourceSelector = createSelector([regionSelectors.streamsSelector], streams => {
  if (isEmpty(streams)) {
    return null
  }
...
export const palSourceSelector = createSelector([regionSelectors.palsSelector], pals => {
  if (isEmpty(pals)) {
    return null
  }
...
export const restrictionSectionSourceSelector = createSelector(
  ...
  restrictionSections => {
    ...
    const restrictionSectionsSource = sourceGenerator(
      RESTRICTION_SECTIONS_SOURCE_ID,
      restrictionSections
    )
...
export const streamAccessPointsSourceSelector = createSelector(
  ...
  streamAccessPoints => {
    ...
    const streamAccessPointsSource = sourceGenerator(
